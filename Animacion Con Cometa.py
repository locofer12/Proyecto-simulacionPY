...
def concometa(i):
    ...
    centroMasaX = (MasaEstrella1*x1[i] + MasaEstrella2*x2[i] + masaPlaneta1*x3[i]+ masaPlaneta2*x4[i] + MasaCometa*x5[i])/(MasaEstrella1 + MasaEstrella2
                                                                                                   + masaPlaneta1 + masaPlaneta2+ MasaCometa)
    ...
    xc = (centroMasaX + x5[:3*i])*DA
    yc =  (centroMasaY + y5[:3*i])*DA
    zc =  (centroMasaZ + z5[:3*i])*DA
    ...
    ax.scatter3D(xc,yc,zc ,s = 0.5 ,color = 'Purple', label = "cometa ") #cometa 1
